...
b. we can reset completely session, deleting as usual with destroy_record + add the deletion of image_reference there
 ...
 - Make pieces preload before displaying the puzzle view to make them appear all at ones not one after another

 - Resize those gallery images which height is greater than their width

 - couch_seed should parse some json/yml file instead to load those values

 - the app load is very slowwwww

 - something is wrong with the board, it should fit cells correctly, number of cells per width = image.columns
 number of cells per height = image.row 