...
SATERDAY:
 3. Cut all images, save pieces to the db, in the meantime:
  ...
  - Define proportion of and image/ or natural width/height to dispalay them properly

 17. Cut all of the images from my collection wisely
 18. Beautify Cover div should have a pseudo-button
 19. RELASE 1.1 - time should be limitted and there should be a time out to get user more competitive
 ...
 3. Fix bug with the cover not overlapping the pieces
 6. background bug fixing

         $('body').append('<div class= mark-right-top ></div>')
    $('body').append('<div class= mark-left-bottom ></div>')

    actual_mark_offset_y = end_point.top_y + $(@el).height() - 14 
    
    $('.mark-left-bottom').css('position','absolute')
     .css('left',"#{end_point.left_x}px")
     .css('top',"#{actual_mark_offset_y}px")
     .css('z-index', '20')    

    $('.mark-right-top').css('position','absolute')
     .css('left',"#{actual_mark_offset_x}px")
     .css('top',"#{end_point.top_y}px")
     .css('z-index', '20')
