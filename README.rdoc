...
=== Backlog:
 * On image select, user will see the board and pieces. On change we send a request to backend, it makes certain Session to the pieces of puzzle and divs, stores it as a metadata as a whole new Session class, and returns the pieces. On each refresh backbone than requests only Sessions out of pieces and divs.
 5. Pry
 9. Beautify game
 11. Add divs coordinates from below the board.
 14. The dragged piece cannot overlap any other after dragend otherwise it is dragged near or backard

SATERDAY:
 3. Cut all images, save pieces to the db, in the meantime:
  - Complicated image pieces should be bigger, easy one pieces should a bit be smaller
  - Define weather an image is complicates or easy
  - Define proportion of and image/ or natural width/height to display them properly

 17. Cut all of the images from my collection wisely
 18. Beautify Cover div should have a pseudo-button
 19. RELASE 1.1 - time should be limited and there should be a time out to get user more competitive
 Some score rating should be present as well...perhaps...
 20. RELEASE 1.2 - user should be able to add some new images to gallery
 21. RELEASE 1.3 - pieces should resemble to puzzle curly pieces no squares anymore

 - For when the puzzle is solved and the Back to Gallery is clicked:
  a. we can apply the same workflow as for Back to Gallery when the puzzle is not yet finished
and delete completely the solved puzzle at the close  button click on the tiny corner img 
or by choosing/clicking on any other image to start new game
b. we can reset completely session, deleting as usual with destroy_record + add the deletion of image_reference there

 - Either improve slow performance or show a progress bar whenever images or pieces are loaded 

 - Refactor session controller and hide method calls in session model, encapsulate some

 - Background job for this one:
    #$redis.flushdb
    #p '-----------------'
    #p $redis.keys('*')

 - Separately write method that will assign width and height for every single image in the db. Run it
 - After its completion refactor image.js.coffee and current_puzzle.js.coffee

 - Make each piece bigger than 50?, also depending on image size/complexity piece could be of two different sizes, bigger ones and smaller ones.
 
 - Essentially, we could either leave the natural image size/ crop it, and resize pieces to minify on frontend. OR We could resize it to minify on backend, its real size will be minified, crop it and represent pieces on their natural size on frontend.

 Smaller picture means less memory consuming and faster browser uploading process.
 NOTE: when it comes to gallery, images should be minified on frontend too, to fit the gallery proportions anyways, so not to count gallery when choosing one of the above workflows. 

 - Make board be formed of a correct number of cells, not 7*13, but the correct one

 - Highlight piece when it is matched using opacity?