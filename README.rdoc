...
b. we can reset completely session, deleting as usual with destroy_record + add the deletion of image_reference there
 ...
 - bug overlapping two pieces simultaneously

 - Background job for this one: there probably no need since session is no longer passed accrossed the network, but stored in a class var aka state var

 - OPTIMIZE THE APP:

 - review rendering of every view and resources, request image from redis as well if needed
