...
=== Backlog:
 * Frontend: invent a way of maintain the primary layout every time the page is refreshed and a call is sent to back end, store states
 ...
 * On image select, user will see the board and pieces. On change we send a request to backend, it makes certain state to the pieces of puzzle and divs, stores it as a metadata as a whole new state class, and returns the pieces. On each refresh backbone than requests only states out of pieces and divs.
 1. Fix divs random selector and force it select only unique divs
 2. Instead of dropdown, user should see a gallery of pictures, on picture click, they disappear and pieces are displayed
