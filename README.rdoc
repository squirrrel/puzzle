...
SATERDAY:
 ...
 RELEASE 1.4 - User story: as a user, I should be able to match any number of pieces everywhere on the screen
 and place them to the board, if the entire pieces tandem is matched with the board, then it is frozen,
 otherwise it can be dragged elsewhere where there is enough space? Possibly, a limited number of pieces can be matched that way in order to make them more movable and not blocked by other pieces.
...
b. we can reset completely session, deleting as usual with destroy_record + add the deletion of image_reference there
 ...
 - each model should be able to access only their own appropriate method - work on couch_seeds.rb

 - the app load is very slow

 - one piece should be put in its place in order to make puzzle solution easier and obvious.

 - 1. fix bug with columns and image reference with bord view.

 - 3. session and remote_ip could be put in a variable or any other container in order not to pass them over and over again.